...
ConVar y_spt_vag_search_portal("y_spt_vag_search_portal",
                               ...
                               "  - overlay: uses same portal as spt_overlay_portal\n" SPT_PORTAL_SELECT_DESCRIPTION);
...
void VagSearcher::StartSearch()
{
	...
	const char* searchPortalString = y_spt_vag_search_portal.GetString();
	bool allowAutoSearch = false;
	if (stricmp(searchPortalString, "overlay") == 0)
	{
		searchPortalString = _y_spt_overlay_portal.GetString();
		allowAutoSearch = true; // only allow auto search if copying auto search from overlay
	}
	enter_portal = getPortal(searchPortalString, true, allowAutoSearch);
